...
const saveJSON = key => key && localStorage.setItem(key, JSON.stringify(data));
...
const CartProvider = ({ children }) => {
    ...
    const addItem = useCallback(price => setItems(prices => prices.concat([prices, price]))); // WHAT??
    const removeItem = useCallback(
        id => setItems (prices => prices.filter(price => prices.id !== id)),
        []
    )
